...
``` r
make_panel_survival_condition <- function(df = dat, dpi) {
  ggplot(df, aes(x = condition, y = survival, color = condition)) +
    geom_point(
      size = 3,
      alpha = 0.8,
      position = position_jitter(width = 0.2, height = 0)
...
make_panel_survival_condition(dpi = view_dpi)
```

![](why-randomize_plots_files/figure-commonmark/unnamed-chunk-8-1.png)

``` r
p <- make_panel_survival_condition(dpi = plot_dpi)
...
make_panel_survival_condition(view_dpi, df = dat_rand)
```

![](why-randomize_plots_files/figure-commonmark/unnamed-chunk-11-1.png)

``` r
p <- make_panel_survival_condition(plot_dpi, df = dat_rand)
...
ggsave(
  "plot_survival_condition_rand.png",
  p,
  width = plot_width,
  height = plot_height,
  units = "in",
